...
def generate_video(prompt: str, seconds: int = 6) -> bytes | None:
    ...
    if raw is None:
        return None
    try:
        return _finish_video(raw, prompt)
    except Exception:
...
def _finish_video(mp4_bytes: bytes, prompt: str) -> bytes:
    """Real per-frame watermark (ffmpeg's drawtext filter — burns the
    mark into every single frame's actual pixels, the video equivalent
    of _watermark_and_tag_image's Pillow compositing, not a prompt
    instruction to the model) + real narration (Cloudflare TTS, in
    place of gTTS's unofficial/timeout-prone endpoint the ModelScope
    path still uses) + real MP4 metadata atoms (ffmpeg -metadata, the
    honest ID3-equivalent for MP4 — same reasoning as
    modelscope-studio/app.py's _add_video_ownership_metadata)."""
    import os
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        narration_bytes = generate_speech(prompt)
        if narration_bytes:
            ...
            try:
                ...
                logger.exception("cloudflare-ai: narration mux failed — delivering the silent (but still watermarked) video instead")
