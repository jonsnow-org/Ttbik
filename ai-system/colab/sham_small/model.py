...
sham_small/verify_architecture.py for an independent, non-torch numpy
...
sham_small/image_tokenizer.py's own docstring for the full reasoning
...
class ShamSmallConfig:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "ShamSmallConfig":
        return cls(**data)
...
class Attention(nn.Module):
    def __init__(self, cfg: ShamSmallConfig):
        super().__init__()
...
def build_default_model() -> ShamSmall:
    ...
    independently in sham_small/verify_architecture.py without needing
    torch/GPU)."""
    cfg = ShamSmallConfig(
    ...
    return ShamSmall(cfg)


if __name__ == "__main__":
    model = build_default_model()
    n_params = model.count_parameters()
    print(f"ShamSmall parameter count: {n_params:,} ({n_params / 1e6:.1f}M)")
