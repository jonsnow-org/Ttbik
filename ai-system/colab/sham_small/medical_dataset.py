...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        model_cfg = ShamSmallConfig(vocab_size=42256, d_model=32, n_layers=2, n_heads=4, n_kv_heads=2, mlp_hidden=64, max_seq_len=1024)
        model = ShamSmall(model_cfg)
        logits, loss = model(input_ids, labels=labels)
        assert torch.isfinite(loss), f"loss is not finite: {loss}"
        print(f"end-to-end OK: real provenance-checked medical clip -> dataset -> collator -> "
              f"ShamSmall.forward() -> finite loss ({loss.item():.4f}).")
