"""
Nova Small — real per-organization API keys, replacing the single
...
(data_acquisition.py, gather_knowledge.py): a new NovaSmallApiKey table
...
def generate_api_key() -> str:
    ...
    return f"nova_org_{secrets.token_urlsafe(32)}"
...
class OrganizationKeyStore:
    ...
    def __init__(self, db_path: str = "nova_small_api_keys.db"):
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert key_a.startswith("nova_org_") and len(key_a) > 40, f"key doesn't look like real high-entropy secret: {key_a}"
        ...
        assert store.verify_api_key("nova_org_totally-made-up-wrong-key") is None
