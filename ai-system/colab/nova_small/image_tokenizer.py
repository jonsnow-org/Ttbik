"""
Nova Small — image tokenizer (a small VQ-VAE), the "translator" between
raw pixels and the discrete visual tokens NovaSmall's own brain
...
reasoning lives entirely in NovaSmall's own transformer, which learns
...
count_parameters() in this file and nova_small/verify_image_tokenizer.py
...
class VectorQuantizer(nn.Module):
    ...
    NovaSmall's own transformer will be trained to predict — the actual
...
class ImageTokenizer(nn.Module):
    ...
    def encode(self, images: torch.Tensor) -> torch.Tensor:
        ...
        integer token ids — what NovaSmall's own transformer is trained to predict
