...
from model import NovaSmallConfig, NovaSmall


def verify_kv_cache() -> None:
    torch.manual_seed(0)
    cfg = NovaSmallConfig(vocab_size=256, d_model=64, n_layers=4, n_heads=4, n_kv_heads=2, mlp_hidden=128, max_seq_len=64)
    model = NovaSmall(cfg)
    model.eval()
...
def verify_config_roundtrip() -> None:
    cfg = NovaSmallConfig(vocab_size=999, d_model=128, n_layers=3, n_heads=4, n_kv_heads=2, mlp_hidden=256, use_gradient_checkpointing=True)
    restored = NovaSmallConfig.from_dict(cfg.to_dict())
...
def verify_context_extension() -> None:
    torch.manual_seed(2)
    cfg = NovaSmallConfig(vocab_size=256, d_model=64, n_layers=2, n_heads=4, n_kv_heads=2, mlp_hidden=128, max_seq_len=16)
    model = NovaSmall(cfg)
