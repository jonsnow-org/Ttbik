"""
Nova Small — model growth. Owner spec: "احرص على ان تكون بنية النموذج
...
("قابل مع التدريب والتطوير للوصول لاكثر من 7B"). NovaSmallConfig
...
separately below rather than assumed to both work the same way:
  ...
  2. Growing WIDTH (d_model, mlp_hidden, n_heads) or VOCAB: every
     ...
     model.py's own NovaSmall._init_weights() already uses. This is a
...
_INIT_STD = 0.02  # must match NovaSmall._init_weights() in model.py
...
def grow_model(old_model: NovaSmall, new_cfg: NovaSmallConfig) -> NovaSmall:
    """Builds a fresh NovaSmall at new_cfg and copies every weight
    ...
    with torch.no_grad():
        if not (old_cfg.tie_embeddings and new_cfg.tie_embeddings):
            ...
            # them (see NovaSmallConfig/build_default_model in
...
if __name__ == "__main__":
    ...
    base_cfg = NovaSmallConfig(vocab_size=128, d_model=32, n_layers=2, n_heads=4, n_kv_heads=2, mlp_hidden=64, max_seq_len=32)
    base_model = NovaSmall(base_cfg)
    base_model.eval()
    ...
    print("\nAll expansion checks passed — a trained NovaSmall checkpoint can genuinely grow into a "
