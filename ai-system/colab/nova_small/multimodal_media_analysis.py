"""
Nova Small — visual and audio "reading," not just text. Owner's own
...
and audio_tokenizer.py (built earlier) let NovaSmall GENERATE media —
...
Two ways this connects to NovaSmall's OWN training, not just to an
external analyzer forever:
  ...
  2. Because NovaSmall's image/audio spans and text share ONE
     ...
     data is exactly what turns NovaSmall from "only generates media"
