...
export default function HealthCheckForm() {
  ...
  async function copyReport() {
    ...
    const lines = [
      ...
      `الاسم العربي (ar): ${b.botFatherNameAr?.trim() || "غير مضبوط"}`,
      `الوصف العربي (ar): ${b.descriptionAr?.trim() || "غير مضبوط"}`,
      `الوصف المختصر العربي (ar): ${b.shortDescriptionAr?.trim() || "غير مضبوط"}`,
      ...
      {result?.bot && (
        <div className="mt-4 space-y-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
          ...
          <ul className="space-y-1 text-sm text-slate-700">
            ...
            {result.bot.botFatherNameAr?.trim() && (
              <li>اسم BotFather بالعربية (ar): {result.bot.botFatherNameAr}</li>
            )}
            <li>
              الوصف المختصر بالعربية (ar):{" "}
              {result.bot.shortDescriptionAr?.trim()
                ? result.bot.shortDescriptionAr
                : "غير مضبوط في BotFather"}
            </li>
            <li>
              الوصف الكامل بالعربية (ar):{" "}
              {result.bot.descriptionAr?.trim()
                ? result.bot.descriptionAr
                : "غير مضبوط في BotFather"}
            </li>
