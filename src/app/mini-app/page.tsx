...
export default function MiniAppPage() {
  ...
  const profileStats = useMemo(() => {
    if (!profileTargetId) return { posts: 0, clones: 0, likes: 0 };
  ...
  const toggleFollow = (sid: string) => setFollowing((prev) => ({ ...prev, [sid]: !prev[sid] }));
        ...
        {showProfile && (
          <div className="mb-4 overflow-hidden rounded-3xl bg-white shadow-md shadow-blue-900/10">
            ...
            <div className="relative px-4 pb-4">
                ...
                {isOwnProfile ? (
                  ...
                  profileTargetId && (
                    <>
                      <button
                        type="button"
                        onClick={() => toggleFollow(profileTargetId)}
                      ...
                      <a
                        href={`https://t.me/${BOT_USERNAME ? BOT_USERNAME.replace(/^@/, "") : "ArabicAds_bot"}`}
              ...
              {/* أقسام الملف: الكل / فيديو / صوت / صورة */}
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {visible.map((item) => {
              ...
              return (
                <article key={item.id} className="overflow-hidden rounded-3xl bg-white shadow-md ring-1 ring-blue-100">
                  <div className="relative aspect-[16/9] bg-slate-100">
                    ...
                    <span className="absolute left-2 top-2 rounded-full bg-black/45 px-2.5 py-1 text-[11px] font-bold text-white">
                      {typeIcon(item.media_type)} {item.media_type === "audio" || item.media_type === "voice" ? "صوت" : "فيديو"}
                    </span>
                  ...
                  <div className="p-3.5">
                    ...
                    <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-500">
                      <button
                        type="button"
                        onClick={() => openProfile(item.sharer_id, item.sharer_name)}
