...
async function routeMainMenuText(bot: TelegramBot, botRow: BotRow, chatId: number, tgUserId: string, user: { role: string }, text: string) {
  ...
  if (text === "ℹ️ معلومات") {
    ...
    const info =
      `🏥 المساعد الطبي — دليلك الصحي الشامل\n\n` +
      `بوت طبي متكامل يخدم المرضى والصيدليات والعيادات والمشافي، كلٌّ بواجهة مخصصة حسب دوره.\n\n` +
      `━━━━━━━━━━━━━━━\n` +
      `👤 للمرضى:\n` +
      `• 💊 صيدليات مناوبة قريبة — يعرض أقرب الصيدليات المناوبة الآن بناءً على موقعك الجغرافي الحقيقي (GPS)، مرتبة من الأقرب فالأقرب مع عرض المسافة بالكيلومترات.\n` +
      `• 🏥 مستشفيات وعيادات — يعرض أقرب المشافي والعيادات المسجلة مع التخصصات والمواقع.\n` +
      `• 👨‍⚕️ حجز موعد طبيب — احجز موعداً لدى أقرب طبيب متاح في التخصص الذي تريده.\n` +
      `• 🚨 طوارئ — نقاط الطوارئ القريبة منك فوراً.\n` +
      `• 📅 حجوزاتي — تتبع مواعيدك وحالتها.\n\n` +
      `━━━━━━━━━━━━━━━\n` +
      `💊 للصيدليات:\n` +
      `• 🟢 تفعيل/إيقاف المناوبة — فعّل مناوبتك ليجدك المرضى القريبون.\n` +
      `• 📩 أسئلة الروشتات — استقبل أسئلة المرضى عن الأدوية والروشتات مباشرة.\n` +
      `• 📍 تحديث الموقع والهاتف.\n\n` +
      `━━━━━━━━━━━━━━━\n` +
      `🏥 للمشافي والعيادات:\n` +
      `• 📅 حجوزات اليوم — إدارة المواعيد المحجوزة.\n` +
      `• 🏬 الأقسام والأطباء — تنظيم التخصصات والجدول الزمني.\n` +
      `• 📍 تحديث الموقع والهاتف.\n\n` +
      `━━━━━━━━━━━━━━━\n` +
      `🩸 نبض — بنك الدم الفوري (لجميع الحسابات):\n` +
      `• 🆘 أحتاج دماً الآن — انشر طلب دم عاجل بفصيلة المريض والمكان ورقم التواصل. يُبلَّغ فوراً كل متبرع مسجّل متوافق الفصيلة وقريب منك ضمن 50 كم.\n` +
      `• 🔎 طلبات قريبة تحتاجني — يعرض طلبات الدم المفتوحة القريبة منك مرتبة من الأقرب فالأقرب مع المسافة الحقيقية بالكيلومترات، ويُظهر فقط المتوافقة مع فصيلتك.\n` +
      `• 🩸 ملفي كمتبرع — سجّل فصيلتك لتصلك نداءات التبرع تلقائياً. تتبع تبرعاتك واحصل على شارات (🌱 متبرع جديد → 🥉 → 🥈 → 🥇 → 🏆 بطل نبض).\n` +
      `• ⏳ فترة راحة تلقائية 90 يوماً بعد كل تبرع.\n` +
      `• 🔗 رابط مشاركة لكل طلب لنشره في مجموعات منطقتك.\n\n` +
      `━━━━━━━━━━━━━━━\n` +
      `📍 نظام الموقع الجغرافي:\n` +
      `يعتمد البوت على موقعك الحقيقي عبر GPS لإيجاد أقرب الخدمات والمتبرعين. يمكنك تحديث موقعك في أي وقت.\n\n` +
      `🔒 الخصوصية: بياناتك الطبية وموقعك لا تُشارك مع أي طرف.\n\n` +
      `📲 شارك البوت: https://t.me/${me.username}`;
    await bot.api.sendMessage(chatId, info, { reply_markup: mainMenuFor(role) });
    return;
  }
